...
var groundTexture = SKTexture(imageNamed: "ground")
    ...
    var img = SKSpriteNode(imageNamed: "sun")
    var selector = 0
var idioma = String()
    ...
    let antTexture1 = SKTexture(imageNamed: "ant1W")
    let antTexture2 = SKTexture(imageNamed: "ant2W")
    let antTexture3 = SKTexture(imageNamed: "ant3W")
    let antTexture4 = SKTexture(imageNamed: "ant4W")
    let owlTexture1 = SKTexture(imageNamed: "owl1W")
    let owlTexture2 = SKTexture(imageNamed: "owl2W")
    let owlTexture3 = SKTexture(imageNamed: "owl3W")
    let owlTexture4 = SKTexture(imageNamed: "owl4W")
    let owlTexture5 = SKTexture(imageNamed: "owl5W")
    let owlTexture6 = SKTexture(imageNamed: "owl6W")
    ...
    //Arrays
    let Textures = [SKTexture(imageNamed: "sunW"),SKTexture(imageNamed: "moon"),SKTexture(imageNamed: "bird1"),SKTexture(imageNamed: "firefly1"),SKTexture(imageNamed: "cloud"),SKTexture(imageNamed: "rainyCloud"),SKTexture(imageNamed: "rainbow"),SKTexture(imageNamed: "plane"),SKTexture(imageNamed: "boat"),SKTexture(imageNamed: "fish1W"),SKTexture(imageNamed: "crab2"), SKTexture(imageNamed: "polvo1"),SKTexture(imageNamed: "baiacuNormal1"),SKTexture(imageNamed: "submarine1"),SKTexture(imageNamed: "jellyfish1"),SKTexture(imageNamed: "sharkW"),SKTexture(imageNamed: "dog1"),SKTexture(imageNamed: "dogBall"),SKTexture(imageNamed: "spider1"),SKTexture(imageNamed: "ladybug1"),SKTexture(imageNamed: "appleW"),SKTexture(imageNamed: "beeW"),SKTexture(imageNamed: "ant1W"),SKTexture(imageNamed: "owl1W"),SKTexture(imageNamed: "colmeiaW"),SKTexture(imageNamed: "tree"),SKTexture(imageNamed: "snail1")]
    var nomesPt = ["SOL","LUA","PÁSSARO","VAGALUME","NUVEM","CHUVA","ARCO ÍRIS","AVIÃO","BARCO","PEIXE","CARANGUEJO","POLVO","BAIACU","SUBMARINO","ÁGUA VIVA","TUBARÃO","CACHORRO","OSSO","ARANHA","JOANINHA","MAÇÃ","ABELHA","FORMIGA","CORUJA", "COLMÉIA","ÁRVORE","CARACOL"]
     var nomesIn = ["SUN","MOON","BIRD","FIREFLY","CLOUD","RAIN","RAINBOW","AIRPLANE","BOAT","FISH","CRAB","OCTOPUS","PUFFERFISH","SUBMARINE","JELLYFISH","SHARK","DOG","BONE","SPIDER","LADYBUG","APPLE","BEE","ANT","OWL","BEEHIVE","TREE","SNAIL"]
 
    
    let audioIn = ["sun", "moon", "bird", "firefly","cloud", "rain", "rainbow", "airplane","boat","fish","crab","octopus","pufferfish","submarine","jellyfish","shark","dog","bone","spider","ladybug","apple","bee","ant","owl","beehive","tree","snail"]
    let audioPt = ["sol", "lua", "passaro", "vagalume","nuvem", "chuva", "arcoiris", "aviao","barco","peixe","caranguejo","polvo","baiacu","submarino","aguaviva","tubarao","cachorro","osso","aranha","joaninha","maca","abelha","formiga","coruja","colmeia","arvore","caracol"]
    ...
    func letterOrganize(){
     ...
     scene?.removeAllActions()
       ...
       physicsRedo()
        ...
        //ARRUMAR PQ NÃO TENHO AINDA CONHECIMENTO
      //aqui
        if palavraTexto == ["M", "O", "O", "N"]{
          self.letterCube3.physicsBody = nil
            letter3Moves = false
            letterCube3.physicsBody?.isDynamic = false
            letterCube3.position = letter3.position
        }else if palavraTexto == ["F", "I", "R", "E", "F", "L", "Y"]{
            self.letterCube1.physicsBody = nil
            letter1Moves = false
            letterCube1.physicsBody?.isDynamic = false
            letterCube1.position = letter1.position
        }else if palavraTexto == ["A", "I", "R", "P", "L", "A", "N", "E"]{
              self.letterCube1.physicsBody = nil
            letter1Moves = false
            letterCube1.physicsBody?.isDynamic = false
            letterCube1.position = letter1.position
        }else if palavraTexto == ["O", "C", "T", "O", "P", "U", "S"]{
              self.letterCube1.physicsBody = nil
            letter1Moves = false
            letterCube1.physicsBody?.isDynamic = false
            letterCube1.position = letter1.position
        ...
        }else if palavraTexto == ["A", "P", "P", "L", "E"]{
              self.letterCube2.physicsBody = nil
            letter2Moves = false
            letterCube2.physicsBody?.isDynamic = false
            letterCube2.position = letter2.position
        }else if palavraTexto == ["B", "E", "E"]{
              self.letterCube3.physicsBody = nil
            letter3Moves = false
            letterCube3.physicsBody?.isDynamic = false
            letterCube3.position = letter3.position
        }else if palavraTexto == ["B", "E", "E", "H", "I", "V", "E"]{
              self.letterCube2.physicsBody = nil
              self.letterCube7.physicsBody = nil
            letter2Moves = false
            letterCube2.physicsBody?.isDynamic = false
            ...
            letterCube7.physicsBody?.isDynamic = false
            letterCube7.position = letter7.position
        }else if palavraTexto == ["T", "R", "E", "E"]{
              self.letterCube4.physicsBody = nil
            letter4Moves = false
            letterCube4.physicsBody?.isDynamic = false
            letterCube4.position = letter4.position
        }else if palavraTexto == ["P", "Á", "S", "S", "A", "R", "O"]{
              self.letterCube4.physicsBody = nil
            letter4Moves = false
            letterCube4.physicsBody?.isDynamic = false
            letterCube4.position = letter4.position
        }else if palavraTexto == ["V", "A", "G", "A", "L", "U", "M", "E"]{
              self.letterCube2.physicsBody = nil
            letter2Moves = false
            letterCube2.physicsBody?.isDynamic = false
            letterCube2.position = letter2.position
        }else if palavraTexto == ["A", "R", "C", "O", " ", "Í", "R", "I", "S"]{
            self.letterCube2.physicsBody = nil
        ...
        }else if palavraTexto == ["C", "A", "C", "H", "O", "R", "R", "O"]{
              self.letterCube7.physicsBody = nil
            ...
            letterCube3.physicsBody?.isDynamic = false
            letterCube3.position = letter3.position
        }else if palavraTexto == ["A", "R", "A", "N", "H", "A"] {
              self.letterCube1.physicsBody = nil
              self.letterCube6.physicsBody = nil
            letter1Moves = false
            letterCube1.physicsBody?.isDynamic = false
            ...
            letterCube6.physicsBody?.isDynamic = false
            letterCube6.position = letter6.position
        }else if palavraTexto == ["J", "O", "A", "N", "I", "N", "H", "A"] {
              self.letterCube3.physicsBody = nil
              self.letterCube6.physicsBody = nil
            letter3Moves = false
            letterCube3.physicsBody?.isDynamic = false
            ...
            letterCube6.physicsBody?.isDynamic = false
            letterCube6.position = letter6.position
        }else if palavraTexto == ["A", "B", "E", "L", "H", "A"] {
              self.letterCube1.physicsBody = nil
            letter1Moves = false
            letterCube1.physicsBody?.isDynamic = false
            letterCube1.position = letter1.position
        }else if palavraTexto == ["C", "A", "R", "A", "C", "O", "L"] {
              self.letterCube1.physicsBody = nil
              self.letterCube2.physicsBody = nil
            letter1Moves = false
            letterCube1.physicsBody?.isDynamic = false
            ...
            letterCube2.physicsBody?.isDynamic = false
            letterCube2.position = letter2.position
        }else if palavraTexto == ["Á", "R", "V", "O", "R", "E"] {
            self.letterCube2.physicsBody = nil
            letter2Moves = false
            letterCube2.physicsBody?.isDynamic = false
            letterCube2.position = letter2.position
...
//abaxaqui
           ...
           treeParticle.setScale(0.88)
            ...
            wordModeButton.position = CGPoint(x: view.frame.width-view.frame.width/34, y: view.frame.height-view.frame.width/17-view.frame.width/33.6)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        let touch = touches.first!
          let total = Textures.count
           ...
           var sizes = [CGSize(width: (self.view?.frame.width)!/4, height: (self.view?.frame.width)!/4),CGSize(width: (self.view?.frame.width)!/4, height: (self.view?.frame.width)!/4),CGSize(width: (self.view?.frame.width)!*0.366, height: (self.view?.frame.width)!*0.25),CGSize(width: (self.view?.frame.width)!*0.366, height: (self.view?.frame.width)!*0.25),CGSize(width: (self.view?.frame.width)!*0.366, height: (self.view?.frame.width)!*0.25),CGSize(width: (self.view?.frame.width)!*0.366, height: (self.view?.frame.width)!*0.25),CGSize(width: (self.view?.frame.width)!, height: (self.view?.frame.height)!*0.65),CGSize(width: (self.view?.frame.width)!*0.41, height: (self.view?.frame.width)!*0.25),   CGSize(width: (self.view?.frame.width)!*0.24, height: (self.view?.frame.width)!*0.25),CGSize(width: (self.view?.frame.width)!*0.28, height: (self.view?.frame.width)!*0.25),CGSize(width: (self.view?.frame.width)!*0.38*0.8, height: (self.view?.frame.width)!*0.25*0.8),CGSize(width: (self.view?.frame.width)!*0.24, height: (self.view?.frame.width)!*0.25),CGSize(width: (self.view?.frame.width)!*0.3*1.3, height: (self.view?.frame.width)!*0.2*1.3),CGSize(width: (self.view?.frame.width)!*0.41, height: (self.view?.frame.width)!*0.25),CGSize(width: (self.view?.frame.width)!*0.2, height: (self.view?.frame.width)!*0.25),CGSize(width: (self.view?.frame.width)!*0.44, height: (self.view?.frame.width)!*0.25),CGSize(width: (self.view?.frame.width)!*0.2*0.8, height: (self.view?.frame.width)!*0.4*0.8),CGSize(width: (self.view?.frame.width)!*0.3, height: (self.view?.frame.width)!*0.13),CGSize(width: (self.view?.frame.width)!*0.34, height: (self.view?.frame.width)!*0.25), CGSize(width: (self.view?.frame.width)!*0.34*0.92, height: (self.view?.frame.width)!*0.25*0.92),CGSize(width: (self.view?.frame.width)!*0.28*0.9, height: (self.view?.frame.width)!*0.33*0.9),CGSize(width: (self.view?.frame.width)!*0.31*0.9, height: (self.view?.frame.width)!*0.2*0.9),CGSize(width: (self.view?.frame.width)!*0.29*0.9, height: (self.view?.frame.width)!*0.2*0.9),CGSize(width: (self.view?.frame.width)!*0.21*0.9, height: (self.view?.frame.width)!*0.31*0.9),CGSize(width: (self.view?.frame.width)!*0.2*0.9, height: (self.view?.frame.width)!*0.27*0.9),CGSize(width: (self.view?.frame.width)!*0.4, height: (self.view?.frame.width)!*0.5),CGSize(width: (self.view?.frame.width)!*0.35*0.6, height: (self.view?.frame.width)!*0.4*0.6)]
        
         if UIDevice.current.userInterfaceIdiom == .phone {
             sizes = [CGSize(width: (self.view?.frame.width)!*0.8/4, height: (self.view?.frame.width)!*0.8/4),CGSize(width: (self.view?.frame.width)!*0.77/4, height: (self.view?.frame.width)!*0.77/4),CGSize(width: (self.view?.frame.width)!*0.8*0.366, height: (self.view?.frame.width)!*0.8*0.25),CGSize(width: (self.view?.frame.width)!*0.7*0.366, height: (self.view?.frame.width)!*0.7*0.25),CGSize(width: (self.view?.frame.width)!*0.75*0.366, height: (self.view?.frame.width)!*0.75*0.25),CGSize(width: (self.view?.frame.width)!*0.75*0.366, height: (self.view?.frame.width)!*0.75*0.25),CGSize(width: (self.view?.frame.width)! /*aqui antes*/, height: (self.view?.frame.height)!*0.76),CGSize(width: (self.view?.frame.width)!*0.69*0.41, height: (self.view?.frame.width)!*0.69*0.25),   CGSize(width: (self.view?.frame.width)!*0.8*0.24, height: (self.view?.frame.width)!*0.8*0.25),CGSize(width: (self.view?.frame.width)!*0.8*0.28, height: (self.view?.frame.width)!*0.8*0.25),CGSize(width: (self.view?.frame.width)!*0.8*0.38*0.8, height: (self.view?.frame.width)!*0.8*0.25*0.8),CGSize(width: (self.view?.frame.width)!*0.74*0.24, height: (self.view?.frame.width)!*0.74*0.25),CGSize(width: (self.view?.frame.width)!*0.8*0.3*1.3, height: (self.view?.frame.width)!*0.8*0.2*1.3),CGSize(width: (self.view?.frame.width)!*0.8*0.41, height: (self.view?.frame.width)!*0.8*0.25),CGSize(width: (self.view?.frame.width)!*0.8*0.2, height: (self.view?.frame.width)!*0.8*0.25),CGSize(width: (self.view?.frame.width)!*0.8*0.44, height: (self.view?.frame.width)!*0.8*0.25),CGSize(width: (self.view?.frame.width)!*0.65*0.2*0.8, height: (self.view?.frame.width)!*0.65*0.4*0.8)/*17 ->*/,CGSize(width: (self.view?.frame.width)!*0.8*0.3, height: (self.view?.frame.width)!*0.8*0.13),CGSize(width: (self.view?.frame.width)!*0.8*0.34, height: (self.view?.frame.width)!*0.8*0.25), CGSize(width: (self.view?.frame.width)!*0.8*0.34*0.92, height: (self.view?.frame.width)!*0.8*0.25*0.92),CGSize(width: (self.view?.frame.width)!*0.7*0.28*0.9, height: (self.view?.frame.width)!*0.7*0.33*0.9),CGSize(width: (self.view?.frame.width)!*0.8*0.31*0.9, height: (self.view?.frame.width)!*0.8*0.2*0.9),CGSize(width: (self.view?.frame.width)!*0.8*0.29*0.9, height: (self.view?.frame.width)!*0.8*0.2*0.9),CGSize(width: (self.view?.frame.width)!*0.77*0.21*0.9, height: (self.view?.frame.width)!*0.77*0.31*0.9),CGSize(width: (self.view?.frame.width)!*0.8*0.2*0.9, height: (self.view?.frame.width)!*0.8*0.27*0.9),CGSize(width: (self.view?.frame.width)!*0.6*0.4, height: (self.view?.frame.width)!*0.6*0.5),CGSize(width: (self.view?.frame.width)!*0.8*0.35*0.6, height: (self.view?.frame.width)!*0.8*0.4*0.6)]
...
audioRunning = false
    ...
    func animate(){
        ...
        let birdTextures = [birdTexture1,birdTexture2,birdTexture3,birdTexture4,birdTexture5,birdTexture4,birdTexture3,birdTexture2]
        let birdAnimate = SKAction.repeatForever(SKAction.animate(with: birdTextures, timePerFrame: 0.1))
        
        let fireflyTextures = [fireflyTexture1,fireflyTexture2,fireflyTexture3,fireflyTexture4,fireflyTexture5,fireflyTexture4,fireflyTexture3,fireflyTexture2]
        let fireflyAnimate = SKAction.repeatForever(SKAction.animate(with: fireflyTextures, timePerFrame: 0.1))
        
        let spiderTextures = [spiderTexture1,spiderTexture2,spiderTexture3,spiderTexture4,spiderTexture3,spiderTexture2]
        let spiderAnimate = SKAction.repeatForever(SKAction.animate(with: spiderTextures, timePerFrame: 0.11))
        
        let ladybugTextures = [ladybugTexture1,ladybugTexture2,ladybugTexture3,ladybugTexture4,ladybugTexture3,ladybugTexture2]
        let ladybugAnimate = SKAction.repeatForever(SKAction.animate(with: ladybugTextures, timePerFrame: 0.11))
        
        let owlTextures = [owlTexture1,owlTexture2,owlTexture3,owlTexture4,owlTexture5,owlTexture6,owlTexture5,owlTexture4,owlTexture3,owlTexture2]
        let owlAnimate = SKAction.repeatForever(SKAction.animate(with: owlTextures, timePerFrame: 0.15))
        
        let antTextures = [antTexture1,antTexture2,antTexture3,antTexture4,antTexture3,antTexture2]
        let antAnimate = SKAction.repeatForever(SKAction.animate(with: antTextures, timePerFrame: 0.11))
        
        let rainbowAnimation = SKAction.repeatForever(SKAction.sequence([SKAction.fadeAlpha(to: 0.5, duration: 1.3),SKAction.fadeAlpha(to: 1, duration: 1)]))
        ...
        let baiacuAction = SKAction.repeatForever(SKAction.animate(with: baiacuTextures, timePerFrame: 0.12))
          ...
          let crabAction = SKAction.repeatForever(SKAction.animate(with: crabTextures, timePerFrame: 0.12))
           ...
           let cloudAnimate = SKAction.repeatForever(SKAction.sequence([SKAction.moveTo(x: (view?.frame.width)!*0.6, duration: 3), SKAction.moveTo(x: (view?.frame.width)!*0.37, duration: 3)]))
        ...
        let snailTextures = [snailTexture1,snailTexture2,snailTexture3,snailTexture2]
        let snailAnimate = SKAction.repeatForever(SKAction.animate(with: snailTextures, timePerFrame: 0.15))
        ...
        let raboAction = SKAction.repeatForever(SKAction.sequence([SKAction.rotate(toAngle: .pi/15, duration: 0.1),SKAction.rotate(toAngle: -.pi/15, duration: 0.1) ]))
        
        let beeAction = SKAction.repeatForever(SKAction.sequence([SKAction.rotate(toAngle: .pi/15, duration: 0.15),SKAction.rotate(toAngle: -.pi/15, duration: 0.15) ]))
 ...
 rabo.removeFromParent()
        ...
        if selector == 0 {
            ...
            //avião
            img.run(planeAnimate)
        }else if selector == 6 {
            //arco iris
            img.position = CGPoint(x: (view?.frame.width)!*0.5, y: (view?.frame.height)!*0.65)
              if UIDevice.current.userInterfaceIdiom == .phone {
               img.position = CGPoint(x: (view?.frame.width)!*0.5, y: (view?.frame.height)!*0.55)
            }
            img.run(rainbowAnimation)
        }else if selector == 2 {
            //pássaro
            img.run(birdAnimate)
        }else if selector == 1 {
            ...
            //vagalume
            img.run(fireflyAnimate)
        }else if selector == 4 {
            //nuvem
            img.run(cloudAnimate)
        }else if selector == 5 {
          ...
          img.run(spiderAnimate)
        }else if selector == 19 {
            img.run(ladybugAnimate)
        }else if selector == 21 {
            img.run(beeAction)
        }else if selector == 22 {
            img.run(antAnimate)
          ...
          addChild(beeParticle)
            ...
            palavra.position = CGPoint(x: (view?.frame.width)!*0.5, y: (view?.frame.height)!*0.15)
               img.position = CGPoint(x: (view?.frame.width)!*0.5, y: (view?.frame.height)!*0.65)
          addChild(treeParticle)
        }else if selector == 26 {
           img.run(snailAnimate)
        }
